      ...
      class MyHandler implements ClickHandler, KeyUpHandler {
         ...
         private void displayResults() {
            ...
            if (!error) {
               ...
               InterestCalculator interestCalculator = new InterestCalculator(initialPrincipal, monthlyPayment,
                     rateMonthsList);
               double interestPaid = interestCalculator.calculate();
               String result = "" + interestPaid;

               // add the result to the beginning of the previous results so it
               // shows first next render.
               results.add(0, new Results(initialPrincipal, monthlyPayment, rateMonths.getPercentInterestRate(),
                     interestPaid));
      ...
      closeButton.addClickHandler(new ClickHandler() {
         public void onClick(ClickEvent event) {
            ...
            for (Results nextResult : results) {
               RootPanel.get("lastInitialPrincipalFieldContainer")
                     .add(new Label("" + nextResult.getInitialPrincipal()));
